#!/usr/bin/env python3
"""
Qwopus3.5-27B-v3 MLX Server
Port: 8095 | OpenAI-compatible API
"""
...
def generate_completion(
    model, tokenizer, model_name: str, messages: List[Dict[str, str]],
    ...
    full_text = mlx_lm.generate(
        model, tokenizer, prompt=prompt_array,
...
class QwopusHandler(BaseHTTPRequestHandler):
    model = None
    tokenizer = None
    model_name = "unknown"
    ...
    def do_GET(self):
        if self.path == "/health":
            self.send_json({"status": "ok", "model": self.model_name})
    ...
    def _handle_chat_completions(self):
        ...
        try:
            ...
            gen_thread = Thread(target=_generate, daemon=True)
            gen_thread.start()
...
def main():
    parser = argparse.ArgumentParser(description="Qwopus3.5-27B-v3 MLX Server")
    parser.add_argument("--port", type=int, default=8095, help="Server port (default: 8095)")
    parser.add_argument("--model", default="models/qwopus35-27b-v3-mlx-4bit", help="Model path")
    parser.add_argument("--host", default="0.0.0.0", help="Host (default: 0.0.0.0)")
    parser.add_argument("--model-name", default=None, help="Model name for API responses (default: derived from path)")
    ...
    QwopusHandler.model = model
    QwopusHandler.tokenizer = tokenizer
    QwopusHandler.model_name = model_name

    server = ThreadingHTTPServer((args.host, args.port), QwopusHandler)
    print(f"Qwopus server: http://localhost:{args.port}/v1/chat/completions")
